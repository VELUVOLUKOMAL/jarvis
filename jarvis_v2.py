...
def _split_compound_command(text: str) -> list[str]:
    ...
    # 1. Handle "open X and Y" connected nouns under same verb
    if t_lower.startswith(("open ", "launch ", "start ", "run ", "close ", "exit ")):
        words = t_lower.split()
        if "and" in words:
            idx_and = words.index("and")
            first_part = " ".join(words[:idx_and])
            second_part = " ".join(words[idx_and+1:])
            
            # Check if second part is a known target
            from commands.nlp_parser import WEBSITE_MAP, APP_ALIASES, FOLDER_NAMES
            from commands.app_launcher import APP_MAP
            known_targets = set(WEBSITE_MAP.keys()) | set(APP_ALIASES.keys()) | FOLDER_NAMES | set(APP_MAP.keys())
            
            if second_part in known_targets:
                verb = words[0]
                first_cmd = text[:text.lower().find(" and ")].strip()
                second_cmd = f"{verb} {text[text.lower().find(' and ') + 5:].strip()}"
                rest = _split_compound_command(second_cmd)
                return [first_cmd] + rest

    # 2. Split on action/then markers
...
def _execute_single_command(text: str) -> None:
    ...
    if intent == "create_folder":
        ...
        ok, msg = create_folder(params["folder"], params.get("location", ""))
        speak(msg)
        return
...
def wake_and_listen(device_idx: int, source: str) -> None:
    ...
    if current_state != State.SLEEPING:
        if source == "manual" and current_state == State.ACTING:
            log.info("[manual] Interrupting acting state to listen for new command.")
            set_state(State.SLEEPING)
        else:
            log.info("[%s] Already awake (state: %s) — ignoring duplicate trigger.", source, current_state.value)
            return
