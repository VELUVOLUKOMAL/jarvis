...
def speak(text: str) -> None:
    """Speak text — ElevenLabs if available, else offline TTS."""
    ...
    try:
        ...
        chunks = client.text_to_speech.convert(
            voice_id=EL_VOICE_ID, text=text, model_id=EL_MODEL, output_format=EL_FORMAT)
        raw = b"".join(chunks)
    except Exception as e:
        log.warning("ElevenLabs failed: %s — using offline TTS.", e)
        speak_offline(text)
        return
    if not raw:
        speak_offline(text)
...
def listen_for_speech(device_idx: int, timeout: float = 6.0, after_speech_silence: float = 1.2) -> str | None:
    ...
    # Initialize history with a low noise default instead of calibrating dynamically here.
    # Calibrating right after speaking WAKE_RESPONSE captures speaker feedback/echo,
    # which incorrectly inflates the threshold and ignores the user's next spoken command.
    vad.history = [MIN_SPEECH_RMS / 3.0] * 30
...
def handle_command(text: str) -> None:
    ...
    if intent == "jarvis_off":
        ...
        speak("Turning off. Say 'Jarvis on' to activate me again, sir.")
        return
...
def wake_and_listen(device_idx: int, source: str) -> None:
    """Called when wake word or clap detected. Listens for a command."""
    if get_state() != State.SLEEPING:
        log.info("[%s] Already awake — ignoring duplicate trigger.", source)
        return
    if not is_jarvis_active():
        log.info("[%s] Wake request ignored: Jarvis is currently deactivated.", source)
        return

    set_state(State.AWAKE)
    try:
        speak(WAKE_RESPONSE)
        log.info("Listening for command (%.0fs timeout)...", COMMAND_TIMEOUT)
        text = listen_for_speech(device_idx, timeout=COMMAND_TIMEOUT)
        if not text:
            speak("I didn't catch that. I'll be here when you need me.")
            return
...
def _check_wake_word(frames: list, device_idx: int) -> None:
    ...
    if any(word in text for word in WAKE_WORDS) and any(off_w in text for off_w in ["off", "deactivate", "go offline", "turn off", "stop"]):
        set_jarvis_active(False)
        log.info("Jarvis deactivated by voice command!")
        threading.Thread(
            target=speak,
            args=("Turning off. Say 'Jarvis on' to activate me again, sir.",),
            daemon=True,
        ).start()
        return
    ...
    for word in WAKE_WORDS:
        if word in text:
            log.info("Wake word '%s' detected!", word)
            threading.Thread(
    ...
    try:
        with sd.InputStream(device=device_idx, samplerate=SAMPLE_RATE,
            ...
            while True:
                ...
                # Skip clap detection when awake/acting or deactivated
                if get_state() != State.SLEEPING or not is_jarvis_active():
                    time.sleep(0.1)
                    continue
